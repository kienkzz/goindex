document.write(`
<style>
*{box-sizing:border-box}
h1{border-bottom:1px solid silver;margin-bottom:10px;padding-bottom:10px;white-space:nowrap}
table{border-collapse:collapse;font-family:Consolas,monaco,monospace}
th{font-weight:700}
.file-name{text-align:left}
.file-type{text-align:center}
.file-size{padding-left:4em}
.file-date-created,
.file-date-modified{padding-left:2em}
.file-date-created,
.file-date-modified,
.file-size{text-align:end;white-space:nowrap}
.icon{padding-left:1.5em;text-decoration:none}
.icon:hover{text-decoration:underline}
.icon-file{background:url(data:image/png;base64,) left top no-repeat}
.icon-dir{background:url(data:image/png;base64,) left top no-repeat}
.icon-up{background:url(data:image/png;base64,) left top no-repeat}
.icon-text{background:url(data:image/gif;base64,R0lGODlhEAAQAKL/AP///zk5OWtra4yMjMDAwM7Ozufn5wAAACH5BAEAAAQALAAAAAAQABAAAANFSLrcvuMVQGsxsc1KJ9ZcVxREpkxBmgJqwHhGbFCK8LJtZdgLLMcU3gnXWgkJvt/siCoab0oZgEksTm8hzrEk6Hq9rkUCADs=) left top no-repeat}
.icon-audio{background:url(data:image/gif;base64,R0lGODlhEAAQAKL/ADk5OWtra4yMjMDAwM7OzgAAAAAAAAAAACH5BAEAAAMALAAAAAAQABAAAAM8OLpM/C8AAZcTIIOqsM6cFoxglUlfKKLqNBAlBBKCpw6j/dQZEeAxRQ7kCfqAG9oIUtwoCI6H8seRRqsJADs=) left top no-repeat}
.icon-7z{background:url(data:image/png;base64,) left top no-repeat}
.icon-zip{background:url(data:image/png;base64,) left top no-repeat}
.icon-mid{background:url(data:image/gif;base64,) left top no-repeat}
.icon-xml{background:url(data:image/png;base64,) left top no-repeat}
.icon-md{background:url(data:image/gif;base64,) left top no-repeat}
.icon-pdf{background:url(data:image/png;base64,) left top no-repeat}
.icon-bat{background:url(data:image/png;base64,) left top no-repeat}
.icon-html{background:url(data:image/png;base64,) left top no-repeat}
.icon-rar{background:url(data:image/png;base64,) left top no-repeat}
</style>`);


// .icon-image{background:url() left top no-repeat}
// .icon-video{background:url() left top no-repeat}
//
//
// .icon-pdf{background:url() left top no-repeat}
// .icon-msword{background:url() left top no-repeat}

// .icon-XYZ{background:url() left top no-repeat}

// 初始化页面，并载入必要资源
function init(){
    document.siteName = $('title').html();
    $('body').addClass("mdui-theme-primary-blue-grey mdui-theme-accent-blue");
    var html = `
    <h1 id="heading">Index of <?php echo urldecode($path);?></h1>
    <table id="table">
    </table>
	`;
    $('body').html(html);
}

function render(path){
	if(path.indexOf("?") > 0){
		path = path.substr(0,path.indexOf("?"));
	}
    title(path);
    nav(path);
    if(path.substr(-1) == '/'){
    	list(path);
    }else{
	    file(path); // This function is not defined in the provided code, assuming it's for displaying single file info.
                  // If it also generates links from names, it might need similar encoding.
                  // However, the primary issue is usually with listing.
    }
}


// 渲染 title
function title(path){
    path = decodeURI(path); // decodeURI is for display purposes, it's fine.
    $('title').html(document.siteName+' - '+path);
}

// 渲染导航栏
function nav(path){
	path = decodeURI(path); // decodeURI is for display purposes, it's fine.
    $('#heading').html('Index of '+path);
}

// 渲染文件列表
function list(path){
	var content = `
<tr><th class="file-name">Name</th><th class="file-size">Size</th><th class="file-date-modified">Date Modified</th><th class="file-type">Type</th></tr>
	`;

	if(path != '/'){
		var up = path.split('/');
		up.pop();up.pop();
		up = up.join('/')+'/';
		// For ".." link, the path segments are usually already "clean" or generated by server logic.
		// If 'up' could contain special characters from folder names, those parts would need encoding too when 'up' was constructed.
		// However, the current logic for 'up' seems to be navigating directory structures, less likely to hit '#' issues directly in 'up' itself
		// unless a folder name itself has '#', which would also need encoding when that folder link was first created.
		content += `
<tr>
	<td class="file-name">
		<a class="icon icon-up folder" href="${up}">..</a>
	</td>
	<td class="file-size"></td>
	<td class="file-date-modified"></td>
</tr>
		`;
	}
	$('#table').html(content);

    var password = localStorage.getItem('password'+path);

    $.post(path,'{"password":"'+password+'"}', function(data,status){
        var obj = jQuery.parseJSON(data);
        if(typeof obj != 'null' && obj.hasOwnProperty('error') && obj.error.code == '401'){
            var pass = prompt("password","");
            localStorage.setItem('password'+path, pass);
            if(pass != null && pass != ""){
                list(path);
            }else{
                history.go(-1);
            }
        }else if(typeof obj != 'null'){
            list_files(path,obj.files); // Calling the modified list_files
        }
    });
}

function list_files(path,files){
    var html = ""; // Changed to var for consistency with original code style
    for(var i in files){ // Changed to var for consistency
        var item = files[i];
        var p_name_encoded = encodeURIComponent(item.name); // *** THE KEY CHANGE IS HERE ***

        if(item['size']==undefined){
            item['size'] = "";
        }
        item['modifiedTime'] = utc2beijing(item['modifiedTime']);
        item['size'] = formatFileSize(item['size']);

        if(item['mimeType'] == 'application/vnd.google-apps.folder'){
        	var p = path + p_name_encoded + '/'; // Use encoded name for href
            html +=`
				<tr>
					<td class="file-name"><a class="icon icon-dir folder" href="${p}">${item.name}/</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'audio/mid'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-mid" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'audio/mpeg'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-audio" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'image/jpeg'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-image" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
                    <td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'video/mpeg'){ // Assuming you have .icon-video style
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-video" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-7z-compressed'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-7z" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-zip-compressed'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-zip" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'text/plain'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-text" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'text/xml'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-xml" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-rar'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-rar" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'text/html'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-html" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/pdf'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-pdf" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
                    <td class="file-date-modified">${item['mimeType']}</td>
                    <td class="file-date-modified">${item['iconLink'] || ''}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'text/markdown'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-md" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-msdos-program'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-bat" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-msdownload'){ // Assuming icon-x-msdownload is defined or falls back
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-x-msdownload" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/x-bittorrent'){ // Assuming icon-application/x-bittorrent is defined or falls back
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-application/x-bittorrent" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/vnd.google-apps.spreadsheet'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-google.spreadsheet" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
                    <td class="file-date-modified">application/google.spreadsheet</td>
                    <td class="file-date-modified">${item['iconLink'] || ''}</td>
				</tr>
            `;
        } else if(item['mimeType'] == 'application/vnd.google-apps.presentation'){
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-google.presentation" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
					<td class="file-date-modified">${item['modifiedTime']}</td>
					<td class="file-date-modified">application/google.presentation</td>
				</tr>
            `;
        } else{ // Default case for other file types
	        var p = path + p_name_encoded; // Use encoded name for href
            html += `
				<tr>
					<td class="file-name"><a class="icon icon-file" href="${p}">${item.name}</a></td> {/* Display original name */}
					<td class="file-size">${item['size']}</td>
                    <td class="file-date-modified">${item['modifiedTime']}</td>
                    <td class="file-date-modified">${item['mimeType']}</td>
				</tr>
            `;
        }
    }
    $('#table').append(html);
}


//时间转换
function utc2beijing(utc_datetime) {
    // 转为正常的时间格式 年-月-日 时:分:秒
    var T_pos = utc_datetime.indexOf('T');
    var Z_pos = utc_datetime.indexOf('Z');
    var year_month_day = utc_datetime.substr(0,T_pos);
    var hour_minute_second = utc_datetime.substr(T_pos+1,Z_pos-T_pos-1);
    var new_datetime = year_month_day+" "+hour_minute_second; // 2017-03-31 08:02:06

    // 处理成为时间戳
    timestamp = new Date(Date.parse(new_datetime));
    timestamp = timestamp.getTime();
    timestamp = timestamp/1000;

    // 增加8个小时，北京时间比utc时间多八个时区
    var unixtimestamp = timestamp+8*60*60;

    // 时间戳转为时间
    var unixtimestamp = new Date(unixtimestamp*1000);
    var year = 1900 + unixtimestamp.getYear();
    var month = "0" + (unixtimestamp.getMonth() + 1);
    var date = "0" + unixtimestamp.getDate();
    var hour = "0" + unixtimestamp.getHours();
    var minute = "0" + unixtimestamp.getMinutes();
    var second = "0" + unixtimestamp.getSeconds();
    return year + "-" + month.substring(month.length-2, month.length)  + "-" + date.substring(date.length-2, date.length)
        + " " + hour.substring(hour.length-2, hour.length) + ":"
        + minute.substring(minute.length-2, minute.length) + ":"
        + second.substring(second.length-2, second.length);
}

// bytes自适应转换到KB,MB,GB
function formatFileSize(bytes) {
    if (bytes>=1000000000) {bytes=(bytes/1000000000).toFixed(2)+' GB';}
    else if (bytes>=1000000)    {bytes=(bytes/1000000).toFixed(2)+' MB';}
    else if (bytes>=1000)       {bytes=(bytes/1000).toFixed(2)+' KB';}
    else if (bytes>1)           {bytes=bytes+' bytes';}
    else if (bytes==1)          {bytes=bytes+' byte';}
    else                        {bytes='';}
    return bytes;
}

// 监听回退事件
window.onpopstate = function(){
    var path = window.location.pathname;
    render(path);
}


$(function(){
    init();
    var path = window.location.pathname;
    $("body").on("click",'.folder',function(){
        var url = $(this).attr('href'); // href is already correctly encoded by list_files
        history.pushState(null, null, url);
        render(url);
        return false;
    });

    render(path);
});
